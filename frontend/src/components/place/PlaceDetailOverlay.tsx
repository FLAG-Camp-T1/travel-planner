...
export default function PlaceDetailOverlay() {
  const activeDetailOverlay = useAppStore((state) => state.activeDetailOverlay);
  ...
  const currentBookmark =
    bookmarks.find((bookmark) => bookmark.googlePlaceId === activeDetailOverlay.placeId) ?? null;
  ...
  const canRenderBookmarkButton =
    currentBookmark !== null || (latitude != null && longitude != null);
  ...
  const handleBookmarkCategoryUpdate = async (category: string | null) => {
    ...
    await updateBookmarkCategory(currentBookmark.bookmarkId, category);
  };
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                  ...
                  {rating != null ? (
                    <span className="inline-flex items-center gap-1 rounded-full bg-amber-50 px-3 py-1 text-xs font-medium text-amber-700">
                      ...
                      {rating.toFixed(1)}
                      {ratingCount ? (
                        <span className="text-amber-600/80">{ratingCount}</span>
                      ) : null}
                    </span>
                  ) : null}
                </div>
        ...
        <>
          {isCategoryDialogOpen ? (
            <BookmarkCategoryDialog
              ...
              initialCategory={currentBookmark.category}
